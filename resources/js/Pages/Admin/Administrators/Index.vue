    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <div>
                    <h2 class="text-xl font-bold text-slate-900 tracking-tight">Administrateurs</h2>
                    <p class="text-xs text-slate-500 font-medium">Gérez les accès à la plateforme (RBAC)</p>
                </div>
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-200">
                                <th class="px-6 py-3 text-[10px] font-black text-slate-500 uppercase tracking-widest">Nom</th>
                                <th class="px-6 py-3 text-[10px] font-black text-slate-500 uppercase tracking-widest">Email</th>
                                <th class="px-6 py-3 text-[10px] font-black text-slate-500 uppercase tracking-widest">Rôles</th>
                            ...
                            <tr v-for="admin in administrators" :key="admin.id" class="hover:bg-slate-50 transition-colors group">
                                <td class="px-6 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="size-7 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-black text-[10px]">
                                            {{ admin.name.charAt(0) }}
                                        </div>
                                        <span class="font-bold text-slate-700 text-sm">{{ admin.name }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-3 text-xs text-slate-500 font-medium">{{ admin.email }}</td>
                                <td class="px-6 py-3">
                                    <div class="flex flex-wrap gap-1" v-if="admin.roles.length > 0">
                                    ...
                                    <div v-else>
                                        <span class="px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider bg-amber-50 text-amber-600 border border-amber-100 animate-pulse">
                                            En attente
                                        </span>
                                    </div>
                                </td>
                                <td class="px-6 py-3 text-right">
                                    <div class="flex items-center justify-end gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button v-if="admin.roles.length === 0" @click="validateAdmin(admin)" 
                                            class="p-1.5 text-amber-500 hover:text-amber-600 transition-colors" title="Valider l'inscription">
                                        ...
                                        <button @click="openEdit(admin)" class="p-1.5 text-slate-400 hover:text-indigo-600 transition-colors">
                                        ...
                                        <button @click="deleteAdmin(admin)" class="p-1.5 text-slate-400 hover:text-rose-600 transition-colors" v-if="$page.props.auth.user.id !== admin.id">
