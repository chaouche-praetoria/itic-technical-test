    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <div>
                    <h2 class="text-xl font-bold text-slate-900 tracking-tight">Templates de tests</h2>
                    <p class="text-xs text-slate-500 font-medium">Gérez vos modèles d'évaluations par domaine et niveau.</p>
                </div>
                        ...
                        <table class="w-full text-[13px] border-collapse">
                            <thead>
                                <tr class="bg-slate-50/50 text-slate-400 text-[10px] uppercase tracking-widest font-bold">
                                    <th class="px-6 py-4 text-left border-b border-slate-100 uppercase">Template / Modèle</th>
                                    <th class="px-6 py-4 text-left border-b border-slate-100 uppercase">Domaine & Niveau</th>
                                    <th class="px-6 py-4 text-center border-b border-slate-100 uppercase">Sessions</th>
                                    <th class="px-6 py-4 text-center border-b border-slate-100 uppercase">Statut</th>
                                ...
                                <tr v-for="t in templates.data" :key="t.id" class="hover:bg-slate-50/80 transition-all group">
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col">
                                            <span class="font-bold text-slate-800 text-[13px] mb-0 group-hover:text-indigo-600 transition-colors">{{ t.name }}</span>
                                            <span class="text-[10px] text-slate-400 font-medium line-clamp-1 truncate max-w-xs">{{ t.description || 'Pas de description' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col">
                                            <span class="text-slate-700 font-semibold text-xs">{{ t.domain?.name }}</span>
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end gap-1.5">
                                            <Link :href="route('admin.templates.edit', t.id)" 
                                                class="size-8 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center hover:bg-indigo-600 hover:text-white transition-all shadow-sm shadow-indigo-100/20">
                                            ...
                                            <button @click="deleteTemplate(t.id)" 
                                                class="size-8 rounded-lg bg-rose-50 text-rose-500 flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all shadow-sm shadow-rose-100/20">
